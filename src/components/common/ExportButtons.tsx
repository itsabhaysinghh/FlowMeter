...
export const ExportButtons: React.FC<ExportButtonsProps> = ({ variant = 'compact' }) => {
  ...
  if (variant === 'compact') {
    return (
      <div className="flex items-center gap-2">
        ...
        {status.message && (
          <div className="text-xs font-medium px-2.5 py-1 rounded-lg bg-[#00B4D8]/10 text-[#00B4D8] border border-[#00B4D8]/30 flex items-center gap-1">
  ...
  return (
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 p-4 bg-white dark:bg-dark-card border border-flostat-border dark:border-dark-border rounded-2xl shadow-flostat">
      ...
      {status.message && (
        <div className="w-full sm:w-auto text-xs font-medium px-3 py-1.5 rounded-lg bg-[#00B4D8]/10 text-[#00B4D8] border border-[#00B4D8]/30 flex items-center gap-1.5">
          {status.message.includes('pending') ? (
